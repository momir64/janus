...
Kredenac is an online cabinet for all your notes and files. You unlock it with a passkey, and your email is how you get
one: a magic link registers your first passkey, or a new one if you lose it. This is the web client
for [kredenac.moma.rs](https://kredenac.moma.rs): the pages you sign in through, keep your notes in, and drop your files
into. It is a light frameworkless single-page app in plain HTML, TypeScript and SCSS built using Vite.
...
src/
  ...
  pages/<page>/         one folder per page: page.ts, page.html and page.scss, plus subparts
...
injection short of deliberately going around it. Going without a framework is the same
decision as going without a WebAuthn library on the server: the whole thing stays small enough
to read in one sitting, and everything the thesis claims about it can be pointed at.
...
- Anything the server answers with a 5xx is shown as the server being unreachable, the same
  as a failed fetch. That includes the `503` the backend returns while its secrets vault is
  still locked, which from the browser's side is exactly a server that is not up yet.
...
`vite.config.ts` serves over HTTPS when `../certs/backend.crt` and `backend.key` exist, proxies
`/api` to `https://localhost:8080`, and sets the same security headers the backend sends in
production, so the CSP is exercised during development too. lokot writes the backend's
certificate and key into `$LOKOT_DIR/backend`, so copy that pair into `../certs` when you want
HTTPS locally. `allowedHosts` includes the production hostname so a Cloudflare tunnel can be
pointed at the dev server when debugging against real passkeys, since the RP ID has to match.
`npm run dev:preview` is a plain-HTTP variant on port 5174 with the same proxy.
